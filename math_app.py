...
if generate_btn:
    ...
    try:
        ...
        【重要：問題の被りを完全に排除する】
        リストアップする{num_questions}個のテーマは、互いに**「問題形式」「数値設定」「問う角度」**を明確に変えてください。
        1. **形式の分散**: (例: 式を求める問題、座標を求める問題、グラフを選ぶ問題、文章題など)
        2. **数値・状況の変化**: (例: 係数が正の数・負の数・分数などでバリエーションをつける)
        3. **具体性**: それぞれのテーマには「何をする問題か」を具体的に書いてください。
        
        【出力形式】
        以下のJSON形式のリストのみを返してください。
        ["グラフから式を求める問題 (傾きが正)", "xの値からyを求める計算問題 (負の数を含む)", "速さと時間の文章題"]
        """
    ...
    for i, theme in enumerate(themes):
        ...
        prompt = f"""
        あなたは数学の出題AIです。以下の「指定されたテーマ」に基づいて1問作成してください。
        ...
        【作成のルール】
        1. **テーマの遵守**: 上記の「指定テーマ」の内容に忠実に作成してください。
        2. **数値のランダム性**: 他の問題と数値が被らないように、意図的に複雑な数や端数、あるいはシンプルな整数を使い分けてください。
        3. **オリジナリティ**: よくあるテンプレート問題そのままではなく、少しひねりや具体的な文脈（文章題の場合）を加えてください。
        4. **数式の表記（重要）**: PDF出力のため、LaTeX形式（$$, \\[ \\], \\fracなど）は一切使用しないでください。これらはPDFで正しく表示されません。代わりに通常のテキスト記号（+, -, ×, ÷, /, ^, √）や日本語を使用してください。（例: 1/3, x^2, √5, 3×2）
